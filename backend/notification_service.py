...
Always logs an entry to MongoDB `notification_log` collection so admin panel can
display the activity feed even when external integrations are not configured.
"""
...
def _send_email_sync(to_email: str, subject: str, html_body: str) -> tuple[bool, str]:
    ...
    sender = os.environ.get("SENDER_EMAIL", "notifications@careeros.app")
    if not api_key:
...
def _wrap_email(title: str, body_html: str) -> str:
    return f"""
    <div style="font-family: -apple-system, 'Segoe UI', sans-serif; background:#F7F7F4; padding:32px;">
      <div style="max-width:560px;margin:0 auto;background:#FFFFFF;border:1px solid rgba(17,17,17,0.1);">
        <div style="padding:32px;border-bottom:1px solid rgba(17,17,17,0.1);">
          <div style="font-size:11px;letter-spacing:0.2em;text-transform:uppercase;color:#888;">CareerOS · Campus Intelligence</div>
          <h1 style="margin:8px 0 0;font-size:24px;color:#111;letter-spacing:-0.02em;">{title}</h1>
        </div>
        <div style="padding:32px;color:#333;line-height:1.6;font-size:15px;">{body_html}</div>
        <div style="padding:20px 32px;background:#F7F7F4;color:#888;font-size:12px;">
          You are receiving this because your institution is a Skill Tank partner.
        </div>
      </div>
    </div>
    """
...
async def notify(
    ...
    *,
    ...
    # Email
    if to_email:
        ok, detail = _send_email_sync(to_email, subject, html)
        await _log(db, event, "email", to_email, subject, "sent" if ok else "simulated", detail)

    # Telegram (broadcast to admin chat)
