...
export default function HighRiskStudents({ rosterPath, limit = 6 }) {
  ...
  return (
    <div className="editorial p-8 h-full bg-ink-900 text-bone-100" data-testid="high-risk-students">
      <div className="flex items-center justify-between gap-4">
        <div className="font-mono text-[10px] tracking-[0.24em] text-bone-100/40">AI RISK / HIGH-RISK STUDENTS</div>
        {rosterPath && (
      ...
      {!roster && <div className="text-sm text-bone-100/50 mt-5">Scoring students…</div>}

      <div className="mt-5 space-y-px">
        {weak.map((s, i) => {
          const topRisk = (s.risks || [])[0];
          return (
            <div key={s.student_id || i} className="flex items-center justify-between py-3 border-b border-bone-100/10">
              <div className="min-w-0">
                <div className="font-display tracking-tight truncate">{s.name || s.roll_number || "Student"}</div>
                <div className="font-mono text-[10px] text-bone-100/40 mt-0.5 truncate">
              ...
              <div className="text-right ml-3 shrink-0">
                ...
                <div className="font-mono text-[9px] tracking-[0.16em] uppercase text-bone-100/40">/100</div>
              </div>
            </div>
          );
        })}
        {roster && weak.length === 0 && (
          <div className="text-sm text-bone-100/50">No high-risk students — every tracked student is on track.</div>
        )}
      </div>

      {roster?.needs_intervention != null && (
        <div className="mt-4 font-mono text-[10px] tracking-[0.18em] uppercase text-bone-100/40">
