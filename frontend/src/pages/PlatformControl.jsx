...
import { Building2, Users, Briefcase, Send, Award, Activity } from "lucide-react";
    ...
    <div className="space-y-10">
      <div className="relative border border-line bg-ink-900 text-bone-100 p-10 lg:p-14 overflow-hidden">
        <div className="absolute right-10 top-10 opacity-20 hidden lg:block">
          <div className="font-display text-[12vw] leading-none tracking-tightest text-accent">IO</div>
        </div>
      ...
      {workspace?.actions?.length > 0 && (
        <div className="grid grid-cols-12 gap-3" data-testid="platform-actions">
          {workspace.actions.slice(0, 3).map((action) => (
            <Link key={action.label} to={action.to} className="col-span-12 md:col-span-4 editorial p-6 hover:border-ink-900 transition-colors">
              <div className="font-mono text-[10px] tracking-[0.24em] text-ink-400">{action.priority.toUpperCase()} PRIORITY</div>
