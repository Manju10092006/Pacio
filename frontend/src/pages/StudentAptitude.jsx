...
import { Bookmark, Clock, Flag, Play, Send } from "lucide-react";
  ...
  return (
    <PageTransition className="space-y-10">
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-12 lg:col-span-8 editorial p-0 overflow-hidden" data-testid="aptitude-test-engine">
          {!session ? (
            ...
            <div className="grid grid-cols-12 min-h-[620px]">
              <div className="col-span-12 md:col-span-8 p-8">
                <div className="flex items-center justify-between border-b border-line pb-4">
                  ...
                  <div className="flex items-center gap-2 font-mono text-sm"><Clock size={15} /> {fmt(remaining)}</div>
                </div>
                ...
                <div className="mt-8 space-y-3">
                  {(activeQuestion?.options || []).map((option, index) => (
                    <button
                      key={option}
                      onClick={() => setAnswers({ ...answers, [activeQuestion.question_id]: index })}
                      className={`w-full border px-5 py-4 text-left transition-colors ${answers[activeQuestion.question_id] === index ? "border-ink bg-ink text-bone" : "border-line bg-bone-50 hover:border-ink"}`}
                    >
                      <span className="font-mono text-xs mr-3">{String.fromCharCode(65 + index)}</span>{option}
                    </button>
                  ))}
                </div>
                {analysis && (
              ...
              <div className="col-span-12 md:col-span-4 bg-bone-100/60 border-l border-line p-6">
                <div className="grid grid-cols-5 gap-2">
                  {session.questions.map((q, index) => (
                    <button key={q.question_id} onClick={() => setActive(index)} className={`h-10 border text-xs font-mono ${active === index ? "bg-ink text-bone" : answers[q.question_id] !== undefined ? "bg-accent text-bone border-accent" : "bg-paper border-line"}`}>
                      {index + 1}
                    </button>
                  ))}
                </div>
                <div className="mt-6 grid grid-cols-2 gap-2">
                  <button className="btn justify-center py-2 text-[10px]" onClick={() => setReviewLater((list) => list.includes(activeQuestion.question_id) ? list.filter((id) => id !== activeQuestion.question_id) : [...list, activeQuestion.question_id])}><Flag size={13} /> Review</button>
                  <button className="btn justify-center py-2 text-[10px]" onClick={() => setBookmarks((list) => list.includes(activeQuestion.question_id) ? list.filter((id) => id !== activeQuestion.question_id) : [...list, activeQuestion.question_id])}><Bookmark size={13} /> Bookmark</button>
                </div>
                ...
                <button disabled={!!analysis} onClick={() => submitTest(false)} className="btn mt-6 w-full justify-center py-3 text-xs"><Send size={14} /> Submit Test</button>
              </div>
            </div>
          )}
        </div>
